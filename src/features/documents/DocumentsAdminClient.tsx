...
export default function DocumentsAdminClient({
  ...
  const [pendingDelete, setPendingDelete] = useState<SchoolDocumentRow | null>(
    null
  );
  ...
  function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      const { uploadSchoolDocument } = await import(
        "@/actions/documents-admin"
      );
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          <Button
            ...
            variant="outline"
            onClick={() =>
              window.open(`/api/documents/school/${row.original.id}`, "_blank")
            }
          >
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Yakin hapus dokumen "{pendingDelete?.title}"? Tindakan ini tidak
              bisa dibatalkan.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
