...
export function GradeSheet({ open, onOpenChange, onSubmit, enrollments, subjects, semesters, initialData }: GradeSheetProps) {
  const [loading, setLoading] = React.useState(false)
  const [enrollmentId, setEnrollmentId] = React.useState<string>(initialData ? String(initialData.enrollmentId) : '')
  const [subjectId, setSubjectId] = React.useState<string>(initialData ? String(initialData.subjectId) : '')
  ...
  const [predicate, setPredicate] = React.useState(initialData?.predicate || '')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!enrollmentId || !subjectId || !semesterId || !score.trim() || !grade.trim()) return
    setLoading(true)
    try {
      await onSubmit({
      ...
      <SheetContent side="right" className="w-full sm:max-w-md">
        <form onSubmit={handleSubmit} className="flex flex-col h-full">
          <SheetHeader className="pb-4">
            <SheetTitle>{initialData ? 'Edit Nilai' : 'Input Nilai'}</SheetTitle>
            <SheetDescription>
              {initialData ? 'Perbarui nilai siswa.' : 'Input nilai hasil belajar siswa.'}
            </SheetDescription>
          </SheetHeader>
                ...
                <SelectContent>
                  {enrollments.map((e) => (
                    <SelectItem key={e.id} value={String(e.id)}>
                      {e.studentName || `ID ${e.id}`} — {e.className || ''} ({e.semesterName || ''})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <SheetFooter>
            ...
            <Button type="submit" disabled={loading || !enrollmentId || !subjectId || !semesterId || !score.trim() || !grade.trim()}>
