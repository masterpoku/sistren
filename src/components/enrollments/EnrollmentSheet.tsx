...
export function EnrollmentSheet({
  ...
  semesters,
  initialData,
}: EnrollmentSheetProps) {
  const [loading, setLoading] = React.useState(false);
  const [studentId, setStudentId] = React.useState<string>(
    initialData ? String(initialData.studentId) : ''
  );
  const [classId, setClassId] = React.useState<string>(
    initialData ? String(initialData.classId) : ''
  );
      ...
      <SheetContent side="right" className="w-full sm:max-w-md">
        <form onSubmit={handleSubmit} className="flex flex-col h-full">
          <SheetHeader className="pb-4">
            <SheetTitle>
              {initialData ? 'Edit Pendaftaran' : 'Tambah Pendaftaran'}
            </SheetTitle>
            <SheetDescription>
              {initialData
                ? 'Perbarui data pendaftaran siswa.'
                : 'Daftarkan siswa ke kelas dan semester.'}
            </SheetDescription>
          </SheetHeader>
          ...
          <SheetFooter>
            ...
            <Button
              type="submit"
              disabled={loading || !studentId || !classId || !semesterId}
            >
              {loading
                ? 'Menyimpan...'
                : initialData
