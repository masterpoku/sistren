...
- FR-011: Attendance tracking per session per student. Per-session attendance recording with status values: Present (Hadir), Sick (Sakit), Permit (Izin), Absent (Alpha), Late (Terlambat). Teacher marks attendance per class session. Statuses recorded per student per session. Summary reports for teacher and admin. Schema: `attendance` table with `enrollmentId`, `sessionDate`, `status` enum, `notes`. Status: Implemented — Sprint 29 (2026-06-25). `attendance` schema, server actions, AttendanceTeacherClient/AttendanceStudentClient/AttendanceReportClient, role-dispatched `/attendance` route. Upsert via unique `(enrollmentId, sessionDate)`. Priority: Medium. Verification: Test.
...
- FR-015: Student promotion to next grade level and graduation workflow. Batch class promotion: select origin class → pick target class for next grade → promote all students in origin to target. Individual class transfer (pindah kelas): search/select student → pick new target class. Bulk multi-student transfer also supported. Graduation: mark student enrollment status as graduated (terminal state, enrollment status change). All operations update enrollment records and log to audit_logs. Status: Implemented — Sprint 28 (2026-06-25). `transferStudent`, `bulkTransferStudents`, `promoteClass`, `graduateStudent` server actions in `src/actions/promotion.ts` with audit_logs entries. TransferStudentDialog, BulkTransferDialog, PromoteClassDialog (with preview count), GraduateAction. `/students/promote` route at level 80. Priority: Medium. Verification: Test.
...
- FR-028: RPP (Rencana Pelaksanaan Pembelajaran) document validation. Teachers upload RPP documents for review by Waka Kurikulum. Documents encrypted via existing AES-256-GCM pattern. `rpp_documents` Drizzle table references encrypted file in `school_documents`. Status lifecycle: draft → submitted → approved | rejected | archived. Rejection includes reason, notification via existing push notification system. File formats: PDF, DOCX, images (jpg/png). Permission: `documents.review_rpp` for reviewers. Separate Guru upload page and Waka review queue page. Status: Implemented — Sprint 27 (2026-06-25). `rpp_documents` schema with embedded encryptedData (longtext), 7 server actions, 3 routes (teacher/admin/student), 1 API download route, 3 permissions seeded. Priority: High. Verification: Test.
...
- [ ] All FR-NNN implemented with passing tests or manual inspection — FR-021 not started. FR-003 and FR-004 partial (read-only only). FR-016 partial (KHS download disabled). FR-011, FR-015, FR-028 implemented in Sprint 27-29 (2026-06-25).
...
- 2026-06-21: FR-028 RPP validation spec drafted. Decision: dedicated rpp_documents table referencing school_documents for encrypted file storage. Statuses: draft → submitted → approved | rejected | archived. Permission: documents.review_rpp. Notifications via existing push notification system. Separate upload vs review pages. File formats: PDF, DOCX, images (jpg/png). Context: client requirement (item 2).
- 2026-06-21: FR-015 student class transfer/promotion spec in progress. Batch class changes required. Client request item 3. Context: client requirement (item 3).
