...
async function testUserProfile() {
  ...
  const profileWithUser = await db.query.profiles.findFirst({
    where: (p, { and, eq, isNull }) =>
      and(eq(p.id, profileId), isNull(p.deletedAt)),
    with: {
      user: true,
      major: true,
    },
  ...
  console.log(
    'User role:',
    userWithProfile?.role ? `id=${userWithProfile.role.id}` : 'null'
  );
...
async function testEnrollmentChain() {
  ...
  const enrollmentWithChain = await db.query.enrollments.findFirst({
    where: (e, { and, eq, isNull }) =>
      and(eq(e.id, enrollmentId), isNull(e.deletedAt)),
    with: {
      student: true,
      semester: true,
      class: true,
  ...
  const pass =
    !!enrollmentWithChain?.student &&
    !!enrollmentWithChain?.semester &&
    !!enrollmentWithChain?.class;
  console.log('Result:', pass ? 'PASS ✅' : 'FAIL ❌');
  return pass;
}
