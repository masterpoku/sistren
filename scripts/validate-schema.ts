...
async function run() {
  ...
  // [2] roles
  {
    ...
    const passed = await insertAndVerify(
      ...
      async (id) => {
        await rawConn!.execute(`DELETE FROM roles WHERE id = ?`, [id]);
      }
    );
    results.push(passed);
  }
  ...
  // [3] permissions
  {
    ...
    const passed = await insertAndVerify(
      ...
      async (id) => {
        await rawConn!.execute(`DELETE FROM permissions WHERE id = ?`, [id]);
      }
    );
    results.push(passed);
  }
  ...
  // [4] users
  {
    ...
    if (!insertedIds.roles) {
      console.log('  [4] users [SKIP — roles not ready]');
      results.push(false);
    } else {
      const passed = await insertAndVerify(
        '[4] users',
        async () => {
          const [result] = await rawConn!.execute(
            ...
            [
              userName,
              email,
              'validation_hash_not_real',
              insertedIds.roles,
              `testuser_${TEST_MARKER}`,
            ]
          );
          return Number((result as mysql.ResultSetHeader).insertId);
        },
        async () =>
          queryOne(`SELECT * FROM users WHERE email = ? LIMIT 1`, [email]),
        (row) => {
          if (!row.id || row.name !== userName)
            throw new Error('Data mismatch');
        },
        async (id) => {
          await rawConn!.execute(`DELETE FROM users WHERE id = ?`, [id]);
        }
      );
  ...
  // [5] majors
  {
    ...
    const passed = await insertAndVerify(
      ...
      async () =>
        queryOne(
          `SELECT * FROM majors WHERE name = ? AND deleted_at IS NULL LIMIT 1`,
          [name]
        ),
      (row) => {
        if (!row.id) throw new Error('No id returned');
      ...
      async (id) => {
        await rawConn!.execute(`DELETE FROM majors WHERE id = ?`, [id]);
      }
    );
    results.push(passed);
  }
  ...
  // [6] classes
  {
    ...
    const passed = await insertAndVerify(
      ...
      async () =>
        queryOne(
          `SELECT * FROM classes WHERE code = ? AND deleted_at IS NULL LIMIT 1`,
          [code]
        ),
      (row) => {
        if (!row.id) throw new Error('No id returned');
      ...
      async (id) => {
        await rawConn!.execute(`DELETE FROM classes WHERE id = ?`, [id]);
      }
    );
    results.push(passed);
  }
  ...
  // [7] semesters
  {
    ...
    const passed = await insertAndVerify(
      ...
      async () =>
        queryOne(
          `SELECT * FROM semesters WHERE name = ? AND deleted_at IS NULL LIMIT 1`,
          [name]
        ),
      (row) => {
        if (!row.id) throw new Error('No id returned');
      ...
      async (id) => {
        await rawConn!.execute(`DELETE FROM semesters WHERE id = ?`, [id]);
      }
    );
    results.push(passed);
  }
  ...
  // [8] profiles
  {
    if (!insertedIds.users || !insertedIds.majors) {
      console.log('  [8] profiles [SKIP — users/majors not ready]');
      results.push(false);
    } else {
      let profileId = 0;
      const passed = await insertAndVerify(
        ...
        async () => {
          const [result] = await rawConn!.execute(
            ...
            [
              Number(insertedIds.users),
              `Test Profile ${TEST_MARKER}`,
              'Test City',
              'Test Address',
              'islam',
              'Test Father',
        ...
        async () =>
          queryOne(`SELECT * FROM profiles WHERE user_id = ? LIMIT 1`, [
            Number(insertedIds.users),
          ]),
        (row) => {
          if (!row.id) throw new Error('No id returned');
        ...
        async (id) => {
          await rawConn!.execute(`DELETE FROM profiles WHERE id = ?`, [id]);
        }
      );
      results.push(passed);
    }
  ...
  // [9] accounts
  {
    if (!insertedIds.users) {
      console.log('  [9] accounts [SKIP — users not ready]');
      results.push(false);
    } else {
      ...
      const passed = await insertAndVerify(
        ...
        async () =>
          queryOne(`SELECT * FROM accounts WHERE account_id = ? LIMIT 1`, [
            accountId,
          ]),
        (row) => {
          if (!row.id) throw new Error('No id returned');
        ...
        async (id) => {
          await rawConn!.execute(`DELETE FROM accounts WHERE id = ?`, [id]);
        }
      );
      results.push(passed);
    }
  ...
  {
    if (!insertedIds.users) {
      console.log('  [10] sessions [SKIP — users not ready]');
      results.push(false);
    } else {
      ...
      const passed = await insertAndVerify(
        ...
        async () =>
          queryOne(`SELECT * FROM sessions WHERE token = ? LIMIT 1`, [token]),
        (row) => {
          if (!row.id) throw new Error('No id returned');
        },
        async (id) => {
          await rawConn!.execute(`DELETE FROM sessions WHERE id = ?`, [id]);
        }
      );
      results.push(passed);
    }
  ...
  // [11] verifications
  {
    ...
    const passed = await insertAndVerify(
      ...
      async () =>
        queryOne(`SELECT * FROM verifications WHERE identifier = ? LIMIT 1`, [
          identifier,
        ]),
      (row) => {
        if (!row.identifier) throw new Error('No identifier returned');
      },
      async (id) => {
        await rawConn!.execute(`DELETE FROM verifications WHERE id = ?`, [id]);
      }
    );
    results.push(passed);
  }
  ...
  // [12] role_permissions
  {
    if (!insertedIds.roles || !insertedIds.permissions) {
      console.log('  [12] role_permissions [SKIP]');
      results.push(false);
    } else {
      const passed = await insertAndVerify(
        '[12] role_permissions',
        ...
        (row) => {
          if (!row.role_id) throw new Error('No role_id returned');
        },
        async () => {
          await rawConn!.execute(
  ...
  // [13] user_permissions
  {
    if (!insertedIds.users || !insertedIds.permissions) {
      console.log('  [13] user_permissions [SKIP]');
      results.push(false);
    } else {
      let upId = 0;
      const passed = await insertAndVerify(
        ...
        async () =>
          queryOne(`SELECT * FROM user_permissions WHERE id = ? LIMIT 1`, [
            upId,
          ]),
        (row) => {
          if (!row.id) throw new Error('No id returned');
        ...
        async (id) => {
          await rawConn!.execute(`DELETE FROM user_permissions WHERE id = ?`, [
            id,
          ]);
        }
      );
  ...
  // [14] subjects
  {
    if (!insertedIds.classes || !insertedIds.majors) {
      ...
      const passed = await insertAndVerify(
        ...
        async () =>
          queryOne(
            `SELECT * FROM subjects WHERE code = ? AND deleted_at IS NULL LIMIT 1`,
            [code]
          ),
        (row) => {
          if (!row.id) throw new Error('No id returned');
        ...
        async (id) => {
          await rawConn!.execute(`DELETE FROM subjects WHERE id = ?`, [id]);
        }
      );
      results.push(passed);
    }
  ...
  // [15] enrollments
  {
    if (!insertedIds.users || !insertedIds.semesters || !insertedIds.classes) {
      console.log('  [15] enrollments [SKIP]');
      results.push(false);
    } else {
      let enrollId = 0;
      const passed = await insertAndVerify(
        ...
        async () =>
          queryOne(
            `SELECT * FROM enrollments WHERE id = ? AND deleted_at IS NULL LIMIT 1`,
            [enrollId]
          ),
        (row) => {
          if (!row.id) throw new Error('No id returned');
        ...
        async (id) => {
          await rawConn!.execute(`DELETE FROM enrollments WHERE id = ?`, [id]);
        }
      );
      results.push(passed);
    }
  ...
  // [16] payments
  {
    if (!insertedIds.users) {
      ...
      const passed = await insertAndVerify(
        ...
        async () =>
          queryOne(`SELECT * FROM payments WHERE code = ? LIMIT 1`, [code]),
        (row) => {
          if (!row.id) throw new Error('No id returned');
        },
        async (id) => {
          await rawConn!.execute(`DELETE FROM payments WHERE id = ?`, [id]);
        }
      );
      results.push(passed);
    }
  ...
  {
    if (!insertedIds.users) {
      console.log('  [17] announcements [SKIP]');
      results.push(false);
    } else {
      ...
      const passed = await insertAndVerify(
        ...
        async () =>
          queryOne(
            `SELECT * FROM announcements WHERE title = ? AND deleted_at IS NULL LIMIT 1`,
            [title]
          ),
        (row) => {
          if (!row.id) throw new Error('No id returned');
        ...
        async (id) => {
          await rawConn!.execute(`DELETE FROM announcements WHERE id = ?`, [
            id,
          ]);
        }
      );
  ...
  {
    console.log('  [18] audit_logs (SKIP — table may not be migrated)');
    results.push(true);
  }
  // [19] attachments — may not exist yet
  {
    console.log('  [19] attachments (SKIP — table may not be migrated)');
    results.push(true);
  }
