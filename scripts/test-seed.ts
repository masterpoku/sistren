...
async function count(table: string): Promise<number> {
  const [data] = await db.execute(`SELECT COUNT(*) as cnt FROM ${table}`) as [Row[], unknown];
...
async function main() {
  ...
  console.log(`Before seed — roles: ${rolesBefore}, permissions: ${permsBefore}, role_permissions: ${rolePermsBefore}`);
  ...
  console.log(`After 1st seed — roles: ${rolesAfter1}, permissions: ${permsAfter1}, role_permissions: ${rolePermsAfter1}`);
  ...
  console.log(`After 2nd seed — roles: ${rolesAfter2}, permissions: ${permsAfter2}, role_permissions: ${rolePermsAfter2}`);
  ...
  if (rolesAfter1 !== rolesAfter2) {
    console.error(`FAIL: roles count changed after 2nd seed (${rolesAfter1} → ${rolesAfter2})`);
    pass = false;
  } else {
  ...
  if (permsAfter1 !== permsAfter2) {
    console.error(`FAIL: permissions count changed after 2nd seed (${permsAfter1} → ${permsAfter2})`);
    pass = false;
  } else {
  ...
  if (rolePermsAfter1 !== rolePermsAfter2) {
    console.error(`FAIL: role_permissions count changed after 2nd seed (${rolePermsAfter1} → ${rolePermsAfter2})`);
    pass = false;
  } else {
  ...
  const [roleData] = await db.execute("SELECT name FROM roles ORDER BY id") as [Row[], unknown];
  const roleNames = roleData.map((r) => String(r["name"]));
  console.log(`\nRoles found: ${roleNames.join(", ")}`);

  const expectedRoles = ["superadmin", "administrator", "guru", "siswa", "alumni"];
  ...
  const [permData] = await db.execute("SELECT name FROM permissions ORDER BY name") as [Row[], unknown];
